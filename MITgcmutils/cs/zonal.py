...
def zonalAvg(field,kernel=None,csgrid=None,ny=None):
    ...
    Input arguments:
        field:  Data to average zonally, as generated by MITgcmutils.rdmnc()
            Dimensions should be one of the following:
                ...
                - [nr,nc,6*nc]
                - [nc,6*nc]
    Grid dimensions and averaging interval needs to specified in one of two ways:
    ...
    '''
    # User can either give a precomputed kernel OR a grid dictionary and number of points
    if kernel is None:
        kernel = zonalKernel(csgrid,ny)    
    ...
    if len(inputShape) is 2:
        ...
        if inputShape[0] is not np.size(kernel,axis=0):
            nt = inputShape[0]
            ...
            zonfield = np.sum(kernel * field,axis=(1,2))            
    else:
